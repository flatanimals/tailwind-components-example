<div class="card mb-1">
    <label class="card-header">Item</label>
    <div class="select w-full mb-2">
        <select class="select">
            <option>Classic</option>
            <option>Search</option>
            <option>By Brand</option>
            <option>By Brand Group</option>
            <option selected>By Internal Ref</option>
            <option>By SKU</option>
        </select>
        <div class="caret">
            <svg class="h-4 w-4">
                <use xlink:href="#caret"></use>
            </svg>
        </div>
    </div>
    <div class="flex flex-row mb-2 items-center">
        <input type="text" placeholder="Internal Ref" value="410645" class="rounded w-full mr-1">
        <button class="border-grey-light border rounded p-1">
            <svg class="h-4 w-4"><use xlink:href="#icon-lock"></use></svg>
        </button>
    </div>

    <div class="check-list">
        <label>
            <input type="checkbox" checked>
            <span>Budweiser - 12 Pack Cans <br><small>410645</small></span>
        </label>
    </div>
</div>